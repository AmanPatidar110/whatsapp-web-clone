...
export const AppContextProvider = (props) => {
  ...
  const storageOnComplete = async (mediaFolder, file) => {
    ...
    console.log(
      `Inside storageOnComplete -> originalFile size ${
        file.size / 1024 / 1024
      } MB`
    );
    const metadata = {
    ...
    try {
      ...
      if (file.type.includes("image")) {
        compressedFile = await imageCompression(file, options); // true

        snapshot = await storageRef
          .child(`${mediaFolder}/${uuid()}${compressedFile.name}`)
          .put(compressedFile, metadata);
      } else {
        snapshot = await storageRef
          .child(`${mediaFolder}/${uuid()}${file.name}`)
          .put(file, metadata);
      }
